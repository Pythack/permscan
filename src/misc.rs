...
pub fn check_for_newer_version() {
    ...
    if let Ok(body) = body {
        if let Ok(response) = body.text() {
            ...
            if !latest.is_empty() {
                if latest[0]["tag_name"] != "v2.1.1" {
                    println!("\x1b[93mNew version available: {}! Visit this url: {}\x1b[0m", rem_first(latest[0]["tag_name"].as_str().unwrap(), "v"), latest[0]["url"].as_str().unwrap());
                } else {
